      ...
      >
          ...
          <div
            ...
            <span
              class="text-body1 q-ml-sm">{{
                listOptions.listParams.zt.substring(1, listOptions.listParams.zt.length)
              }}</span>
          </div>
          <!--复核与审核按钮结束-->
        ...
        <template #body-cell-zt="props">
          <td style="text-align: center; width: 110px !important;">
            ...
            <q-chip v-if="props.row.zt === '未审核'" square color="primary" text-color="white">
              {{ props.row.zt }}
            </q-chip>
            <q-chip v-if="props.row.zt === '已完成'" square color="grey" text-color="white">
              {{ props.row.zt }}
            </q-chip>
        ...
        <template #body-cell-opt="props">
          <td style="text-align: center;width: 5% !important;">
            <!--修改按钮开始-->
            <q-btn
              v-if="(listOptions.listEdit==='popup' || listOptions.listEdit==='redirect') && (props.row.zt === '未接收' || props.row.zt === '已退回')"
            ...
            <!--接收按钮开始-->
            <q-btn v-if="listOptions.listEdit==='receive'" icon="check_circle" flat round dense color="teal"
            ...
            <!--退回按钮开始-->
            <q-btn v-if="listOptions.listEdit==='receive'" icon="cancel" flat round dense color="teal"
                ...
                <div class="col-8">
                  <div v-for="jcxm in jcxmList" class="row">
                    ...
                    <div class="col-3 bordered">
                      <q-input placeholder="请输入" :rules="[validate]" dense color="teal" style="width: 165px; height: 50px;"
  ...
  methods: {
    ...
    editItem(props) {
      ...
      //如果是跳转
        ...
        this.$router.push({path: `${this.listOptions.listName.substring(0,this.listOptions.listName.indexOf('wpd'))}scwpd`, query: {id: props.key}})
      }
    },
