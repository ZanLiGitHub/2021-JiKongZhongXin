<template>
  <q-card style="padding: 2%; height: 100%;">
    ...
    <q-form id="form" class="row text-right text-body1" @submit="submitItem" @reset="resetItem">
      ...
      <div class="col-3 row" v-for="(value,key) in wpdOptions.wpdInputs" :key="key" style="margin-bottom: 20px">
        <div class="col-4" v-if="!['zt','id', 'wpdlx', 'jcdx', 'jcxm'].includes(key)" style="margin-top: 7px">
        ...
        <div class="col-8" style="padding-left: 5px" v-if="!['zt','id', 'wpdlx', 'jcdx', 'jcxm'].includes(key)">
          ...
          <!--选择框结束-->
          <!--输入框开始-->
          <q-input v-else v-model=inputs[key] placeholder="请输入" dense outlined color="teal"
                   :disable="key === 'bgbh' && editing"></q-input>
          <!--输入框结束-->
        </div>
      </div>
      <q-separator style="margin: 20px 0"/>
      ...
      <div class="col-5" style="height: 500px; margin-top: 20px">
...
export default {
  ...
  methods: {
    ...
    submitItem() {
      if (this.editing) {
        //后端通信
        this.inputs.zt = '未接收'
        editList(this.wpdOptions.wpdListName, this.inputs.id, this.inputs)
      } else {
        //后端通信
        let wpdlxList = {}
        getList('jcxmList').then(res => {
          ...
          for (let wpdlx in wpdlxList) {
            ...
            addList(this.wpdOptions.wpdListName, inputs)
          }
        })
      }
      //弹出通知
    ...
    //获取选择的历史记录
    submitSelect3(e) {
      getList(this.wpdOptions.wpdListName, {id: e}).then(res => {
        this.inputs = res[0]
        this.inputs.bgbh = ""
  ...
  mounted() {
    ...
    if (this.$route.query.id) {
      //开启编辑模式
      this.editing = true
      getList(this.wpdOptions.wpdListName, {id: this.$route.query.id}).then(res => {
        this.inputs = res[0]
        //用于重置输入
