          ...
          <q-card-section style="max-height: 80vh" class="scroll">
            <div class="row q-ma-md">
              ...
              <div class="col-6 row" v-for="(item,index) in Object.keys(inputs)" :key="index">
                <!--标题开始-->
                <div class="col-3" v-if="!['id', 'bm', 'jyjg'].includes(item)">
                ...
                <div class="col-8">
                  <!--检测类型和科室种类是选择框-->
                  <q-select v-if="['jclx', 'kszl'].includes(item)" outlined dense color="teal" v-model="inputs[item]"
                            @focus="buildOptionList(item)"
                            :options="optionList" label="请选择"/>
                  <!--id和检验结果不显示-->
                  <div v-else-if="['id', 'bm', 'jyjg'].includes(item)"></div>
                  <!--其他都为输入框-->
                  <q-input v-else-if="item==='name'" dense outlined color="teal" v-model=inputs[item] label="请输入" :rules="[validate]"></q-input>
                  <q-input v-else dense outlined color="teal" v-model=inputs[item] label="请输入"></q-input>
                </div>
                <!--输入部分结束-->
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    submitItem() {
      ...
      //如果是新增
        ...
        this.addList(this.inputs)
      }
      //如果是录入结果
      else if (this.listOptions.listEdit === 'result') {
        this.inputs.zt = '未复核'
        this.editList(this.id, this.inputs)
      }
      //如果是多项录入
      else if (this.listOptions.listEdit === 'multipleAdd') {
        //删除inputs中的number属性
      ...
      if (this.listOptions.listEdit !== 'multipleAdd') {
        this.getList(this.listOptions.listParams).then(res => {
          this.list = res
        })
        this.reload()
      }
        ...
        this.getList({id: id}).then(res => {
          ...
          switch (item.zt) {
            case "未接收":
              ...
              //后端通信
              axios.addList(`${this.listOptions.listName.substring(0, this.listOptions.listName.indexOf('wpd'))}jyList`, jybg)
              break;
            case "未复核":
              //设置复核者为当前登陆者
    ...
    rejectItem(id) {
      if (this.popup) {
        this.getList({id: this.id}).then(res => {
          let item = JSON.parse(JSON.stringify(res[0]))
          switch (item.zt) {
            case '已复核':
              item.zt = '未复核';
    ...
    resultItem(id) {
      ...
      this.getList({id: id}).then(res => {
        ...
        for (let jcxm of this.inputs.jcxm) {
          axios.getjcxmList({name: jcxm}).then(res => {
            jcxmList.push(res[0])
          })
        }
    ...
    buildOptionList(item) {
      let optionList = []
      axios.getList(`${item}List`).then(res => {
  ...
  computed: {
    ...
    getList: function () {
      return axios[`get${this.listOptions.listName}`]
    },
    addList: function () {
      return axios[`add${this.listOptions.listName}`]
    },
    deleteList: function () {
      return axios[`delete${this.listOptions.listName}`]
    },
    editList: function () {
      return axios[`edit${this.listOptions.listName}`]
    }
  },
